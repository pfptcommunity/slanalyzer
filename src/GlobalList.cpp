	...
	for (auto& row : parser)
	{
		...
		if (ft == FieldType::UNKNOWN || mt == MatchType::UNKNOWN)
		{
			...
			entry_errors.push_back({
				line_number,
				GetFieldTypeString(ft),
				GetMatchTypeString(mt),
			...
			MatchType::NOT_EQUAL)
		{
			entry_errors.push_back({
				...
				GetFieldTypeString(ft),
				GetMatchTypeString(mt),
				"Negative match condition detected. Please see https://proofpoint.my.site.com/community/s/article/Email-Protection-PPS-PoD-Organizational-Safe-and-Block-List-Best-Practices."
			});
		}
...
{
	return std::accumulate(entries.begin(), entries.end(), 0, [](const std::size_t& a, const Entry& b) -> std::size_t
	{
		return a + b.inbound;
	});
}
