...
namespace Proofpoint
{
	...
	private:
		inline static const std::string FieldTypeStrings[] = {
			"unknown", "$ip", "$host", "$helo", "$rcpt", "$from", "$hfrom"
		};
		inline static const std::string MatchTypeStrings[] = {
			"unknown", "equal", "not_equal", "match", "not_match", "regex", "not_regex", "ip_in_net", "ip_not_in_net",
			"is_in_domainset"
		};
	...
	public:
		...
		[[nodiscard]] const Entry& operator[](std::size_t index) const { return entries.at(index); }
		[[nodiscard]] std::size_t GetLineNumber(std::size_t index) const { return entries.at(index).line_number; }
