...
namespace Proofpoint {
	class Processor {
	public:
		Processor(const SafeList& safelist)
		{
			using
			...
			for (std::size_t i = 0; i<safelist.safe_list.size(); i++) {
				...
				case IP:
					ip.Add(sle->match_type, sle->pattern, i);
					break;
				case HOST:
					host.Add(sle->match_type, sle->pattern, i);
					break;
				case HELO:
					host.Add(sle->match_type, sle->pattern, i);
					break;
				case FROM:
					from.Add(sle->match_type, sle->pattern, i);
					break;
				case HFROM:
					hfrom.Add(sle->match_type, sle->pattern, i);
					break;
				case RCPT:
					rcpt.Add(sle->match_type, sle->pattern, i);
					break;
				case UNKNOWN:
					break;
				}
		...
		void Process(const std::string& ss_file, SafeList& safelist)
		{
			...
			for (auto& row: parser) {
				...
				if (cols==1 && trim_copy(row.at(0)).empty()) continue;
				header = row;
